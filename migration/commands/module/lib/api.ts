...
`
  // @ts-expect-error ts-migrate(2339) FIXME: Property 'modules' does not exist on type 'unknown... Remove this comment to see the full error message
...
export const invokeModule = async (params: any) => {
  ...
  const { invokeModule } = await doQuery(query, { input: removeEmpty({ id, parameters, payload }) })
  return invokeModule
}
